...
#include <dynamic_graph/assert.hpp>
...
inline void ValidateEdge(const UndirectedEdge& edge, int64_t num_vertices) {
	ASSERT_MSG(0 <= edge.first && edge.first < num_vertices && 0 <= edge.second && edge.second < num_vertices, "Edge " << edge << " out of bounds");
}

inline void ValidateVertex(Vertex v, int64_t num_vertices) { ASSERT_MSG(0 <= v && v < num_vertices, "Vertex " << v << " out of bounds"); }

}  // namespace

using namespace detail;

DynamicConnectivity::DynamicConnectivity(int64_t num_vertices) : num_vertices_{num_vertices} {
	ASSERT_MSG_ALWAYS(num_vertices_ > 0,"The number of vertices must be positive");

	const int8_t num_levels = FloorLog2(num_vertices_) + 1;
	
	spanning_forests_ = std::vector<DynamicForest>{ static_cast<std::size_t>(num_levels), DynamicForest(num_vertices_) };
	
	non_tree_adjacency_lists_ = std::vector<std::vector<std::unordered_set<Vertex>>>{
		static_cast<std::size_t>(num_levels),std::vector<std::unordered_set<Vertex>>{
			static_cast<std::size_t>(num_vertices_), std::unordered_set<Vertex>{}
			}
	};
}
...
bool DynamicConnectivity::IsConnected(Vertex u, Vertex v) const { return spanning_forests_[0].IsConnected(u, v); }

bool DynamicConnectivity::HasEdge(const UndirectedEdge& edge) const { return edges_.find(edge) != edges_.end(); }

int64_t DynamicConnectivity::GetSizeOfConnectedComponent(Vertex v) const { return spanning_forests_[0].GetSizeOfTree(v); }

int64_t DynamicConnectivity::GetNumOfEdges() const { return edges_.size(); }

int64_t DynamicConnectivity::GetNumOfVertices() const { 
	int sum = 0;
	for (auto& line : non_tree_adjacency_lists_) {
		for (auto& uns : line) {
			sum += uns.size();
		}
	}
...
void DynamicConnectivity::AddEdgeToAdjacencyList(const UndirectedEdge& edge, detail::Level level) { 
	{
		auto& adj_list_1{non_tree_adjacency_lists_[level][edge.first]};
		if (adj_list_1.empty()) { spanning_forests_[level].MarkVertex(edge.first, true); }
		adj_list_1.emplace(edge.second);
	}
	{ 
		auto& adj_list_2{non_tree_adjacency_lists_[level][edge.second]};
		if (adj_list_2.empty()) { spanning_forests_[level].MarkVertex(edge.second, true); }
		adj_list_2.emplace(edge.first);
	}
}

void DynamicConnectivity::DeleteEdgeFromAdjacencyList(const UndirectedEdge& edge, detail::Level level) {
  {
    auto& adj_list_1{non_tree_adjacency_lists_[level][edge.first]};
    adj_list_1.erase(adj_list_1.find(edge.second));
    if (adj_list_1.empty()) { spanning_forests_[level].MarkVertex(edge.first, false); }
  }
  {
    auto& adj_list_2{non_tree_adjacency_lists_[level][edge.second]};
    adj_list_2.erase(adj_list_2.find(edge.first));
    if (adj_list_2.empty()) { spanning_forests_[level].MarkVertex(edge.second, false); }
  }
}

// Add edge `edge` as a level-0 non-tree edge.
void DynamicConnectivity::AddNonTreeEdge(const UndirectedEdge& edge) {
  const EdgeInfo edge_info{ .level = 0, .type = EdgeType::kNonTree, };
...
void DynamicConnectivity::AddTreeEdge(const UndirectedEdge& edge) {
  const EdgeInfo edge_info{ .level = 0, .type = EdgeType::kTree, };
...
void DynamicConnectivity::AddEdge(const UndirectedEdge& edge) {
  ...
  if (IsConnected(edge.first, edge.second)) { AddNonTreeEdge(edge); } 
  else { AddTreeEdge(edge); }
}
...
void DynamicConnectivity::ReplaceTreeEdge(const UndirectedEdge& edge, Level level) {
  ...
  if (spanning_forest.GetSizeOfTree(u) > spanning_forest.GetSizeOfTree(v)) { std::swap(u, v); }
  ...
  while (true) { const std::optional<const UndirectedEdge> tree_edge { spanning_forest.GetMarkedEdgeInTree(u) };
    
    if (!tree_edge.has_value()) { break; }
  ...
  // edge.
  while (true) {
    const std::optional<const Vertex> vertex_with_incident_edges{ spanning_forest.GetMarkedVertexInTree(u) };
    
    if (!vertex_with_incident_edges.has_value()) { break; }

    std::unordered_set<Vertex>& adj_list{ non_tree_adjacency_lists_[level][*vertex_with_incident_edges] };

    while (!adj_list.empty()) {
      const auto& adj_it{adj_list.begin()};
      const Vertex endpoint{*adj_it};
      const UndirectedEdge replacement_candidate{ *vertex_with_incident_edges, endpoint };
      ...
      if (spanning_forest.IsConnected(u, endpoint)) {
        ...
        for (Level l = level; l >= 0; l--) { spanning_forests_[l].AddEdge(replacement_candidate); }
  ...
  if (level > 0) { ReplaceTreeEdge(edge, level - 1); }// Search on lower level.
  else { } // There is no replacement edge. `u` and `v` are disconnected. 
}

void DynamicConnectivity::DeleteEdge(const UndirectedEdge& edge) {
	ValidateEdge(edge, num_vertices_);
	const auto& edge_it{edges_.find(edge)};
	ASSERT_MSG_ALWAYS( edge_it != edges_.end(), "Edge " << edge << " is not in the graph");
	...
	switch (edge_info.type) {
		case EdgeType::kNonTree:
			...
			break;
		case EdgeType::kTree:
			for (Level l{edge_info.level}; l >= 0; l--) { spanning_forests_[l].DeleteEdge(edge); }
