...
#include <dynamic_graph/sequence.hpp>
...
Element::Element(const std::pair<int64_t, int64_t>& id) : id_{id}, priority_{priority_distribution(random_generator)} {}
...
Element::Element(Element&& other) noexcept
    ...
    , subtree_data_{other.subtree_data_} {
  if (parent_ != nullptr) {
    if (parent_->children_[Direction::kLeft] == &other) { parent_->children_[Direction::kLeft] = this; } 
    
    else { parent_->children_[Direction::kRight] = this; }
  }
  
  if (children_[Direction::kLeft] != nullptr) { children_[Direction::kLeft]->parent_ = this; }
  
  if (children_[Direction::kRight] != nullptr) { children_[Direction::kRight]->parent_ = this; }
}

SubtreeData Element::GetChildSubtreeData(Direction direction) const {
  if (children_[direction] == nullptr) {
    constexpr static SubtreeData empty_subtree_data{ .size = 0, .has_marked = {false, false}, };
    
    return empty_subtree_data;
  } 
  else { return children_[direction]->subtree_data_; }
}
...
Element* Element::GetPredecessor() const {
  ...
  if (current->children_[Direction::kLeft] == nullptr) {
    ...
    while (true) {
      ...
      else if (current->parent_->children_[Direction::kRight] == current) { return current->parent_; } 
      else { current = current->parent_; }
    }
  } 
  else {
    ...
    while (current->children_[Direction::kRight] != nullptr) { current = current->children_[Direction::kRight]; }
    return const_cast<Element*>(current);
  }
}
...
Element* Element::JoinRoots(Element* lesser, Element* greater) {
  ...
  else if (greater == nullptr) { return lesser; }
...
void Element::Mark(int32_t index, bool marked) {
  ...
  while (current != nullptr) {
    ...
    if (current->subtree_data_.has_marked[index] == old_subtree_has_marked) { break; } 
    else { current = current->parent_; }
  }
}
...
void Element::SequenceIds(std::vector<Id>* output) const {
  if (children_[Direction::kLeft] != nullptr) { children_[Direction::kLeft]->SequenceIds(output); }
  
  (*output).push_back(id_);
  
  if (children_[Direction::kRight] != nullptr) { children_[Direction::kRight]->SequenceIds(output); }
}
