...
DynamicForest::DynamicForest(int64_t num_vertices) : num_vertices_(num_vertices) { 
	ASSERT_MSG_ALWAYS( num_vertices_ > 0, "The number of vertices must be positive");

	vertices_.reserve(num_vertices_);
	for (int64_t i = 0; i < num_vertices_; i++) { vertices_.emplace_back(std::make_pair(i, i)); }

	const int64_t max_num_edges{2 * (num_vertices_ - 1)};

	edge_elements_ = std::vector<Element>{ static_cast<std::size_t>(max_num_edges), Element{std::make_pair(-1, -1)}};

	free_edge_elements_.reserve(max_num_edges);

	for (auto& edge : edge_elements_) { free_edge_elements_.emplace_back(&edge); }
...
void DynamicForest::DeleteEdge(const UndirectedEdge& edge) {
  ...
  if (uv_predecessor != nullptr) { uv_predecessor->Split(); }
  Element* const vu_predecessor{vu->GetPredecessor()};
  if (vu_predecessor != nullptr) { vu_predecessor->Split(); }
  if (is_uv_before_vu_in_tour) { Element::Join(uv_predecessor, vu_successor); } 
  else { Element::Join(vu_predecessor, uv_successor); }
...
void DynamicForest::MarkEdge(const UndirectedEdge& edge, bool mark) {
  ValidateEdge(edge, num_vertices_);
  const auto& edge_it{edges_.find(edge)};
  ASSERT_MSG( edge_it != edges_.end(), "Edge " << edge << " is not in the forest.");
...
std::optional<Vertex> DynamicForest::GetMarkedVertexInTree(Vertex v) const {
  ...
  if (edge.has_value()) { return (*edge)->id_.first; } 
  else { return {}; }
}
