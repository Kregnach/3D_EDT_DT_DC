...
#include <dynamic_graph/hash.hpp>
...
struct EdgeInfo { Level level; EdgeType type; };
...
class DynamicConnectivity {
 public:
  ...
  int64_t GetNumOfEdges() const; // Returns the number of edges in the graph
  
  int64_t GetNumOfVertices() const; // Returns the number of vertices in the graph
 ...
 private:
  ...
  std::vector<std::vector<std::unordered_set<Vertex>>> non_tree_adjacency_lists_;
  // All edges in the graph.
  std::unordered_map<UndirectedEdge, detail::EdgeInfo, UndirectedEdgeHash> edges_;
};
